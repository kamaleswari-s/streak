    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '2rem 2.5rem', maxWidth: '1100px', margin: '0 auto' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 style={{ fontFamily: 'var(--font-pixel)', fontSize: '36px', color: 'var(--primary)', marginBottom: '0.5rem' }}>analytics</h1>
          <p style={{ fontSize: '16px', color: 'var(--text-secondary)', marginBottom: '2rem' }}>your real study patterns, no fluff.</p>
          ...
          {loading ? (
            ...
            <div className="glass" style={{ padding: '4rem', textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: 'var(--font-pixel)', fontSize: '20px', color: 'var(--primary)' }}>not enough data yet</div>
              <div style={{ fontSize: '15px', color: 'var(--text-secondary)', marginTop: '8px' }}>complete a few sessions to see your patterns</div>
            </div>
          ) : (
            <>
              ...
              <motion.div className="glass" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ padding: '1.5rem', marginBottom: '2rem' }}>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={data.monthly_trend}>
                    ...
                    <YAxis hide />
                    <Tooltip contentStyle={tooltipStyle} formatter={v => [`${Math.round(v)} mins`, 'studied']} />
                ...
                <motion.div className="glass" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ padding: '1.5rem' }}>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={data.by_day}>
                      <XAxis dataKey="day" tickFormatter={d => d?.trim().slice(0, 3)}
                        tick={{ fontSize: 11, fill: 'var(--text-secondary)' }} axisLine={false} tickLine={false} />
                      <YAxis hide />
                      <Tooltip contentStyle={tooltipStyle} formatter={v => [v, 'sessions']} />
                      <Bar dataKey="count" radius={[4, 4, 0, 0]}>
